...
export class LanguageDto {
  ...
  @IsString() @IsNotEmpty() proficiency: string;
}
...
export class VolunteerWorkDto {
  ...
  @ApiProperty({ example: 'Mentored 15 junior developers in Node.js and API design' })
  @IsString() @IsNotEmpty() description: string;
}

export class ExperienceDto {
  @ApiProperty({ example: 'Google' }) @IsString() @IsNotEmpty() company: string;
  @ApiProperty({ example: 'Senior DevOps Engineer' }) @IsString() @IsNotEmpty() role: string;
  ...
  @IsOptional() @IsArray() @IsString({ each: true }) technologiesUsed?: string[];
...
export class UpdateBasicInfoDto {
  ...
  @IsOptional() @IsArray() @ValidateNested({ each: true }) @Type(() => CertificationDto)
  certifications?: CertificationDto[];
  @ApiPropertyOptional({ type: [ProjectDto] })
  @IsOptional() @IsArray() @ValidateNested({ each: true }) @Type(() => ProjectDto)
  projects?: ProjectDto[];
  ...
  @IsOptional() @IsArray() @ValidateNested({ each: true }) @Type(() => AwardDto)
  awards?: AwardDto[];
  @ApiPropertyOptional({ type: [VolunteerWorkDto] })
  @IsOptional() @IsArray() @ValidateNested({ each: true }) @Type(() => VolunteerWorkDto)
